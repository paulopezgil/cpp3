...
#ifndef TokenizerBASE_H_INCLUDED
#define TokenizerBASE_H_INCLUDED
...
private:
    ...
    size_t (TokenizerBase::*d_get)() = &TokenizerBase::getInput;
...
public:
    TokenizerBase(TokenizerBase const &other)             = delete;
...
protected:
    TokenizerBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    TokenizerBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~TokenizerBase();
...
inline size_t TokenizerBase::Input::lineNr() const
{
    return d_lineNr;
}
inline size_t TokenizerBase::Input::nPending() const
{
    return d_deque.size();
}
inline void TokenizerBase::Input::setPending(size_t size)
{
    d_deque.erase(d_deque.begin(), d_deque.end() - size);
}
inline void TokenizerBase::Input::close()
{
    delete d_in;
...
inline TokenizerBase::~TokenizerBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr TokenizerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline TokenizerBase::StartCondition_ constexpr TokenizerBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr TokenizerBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline TokenizerBase::StartCondition_ TokenizerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void TokenizerBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool TokenizerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr TokenizerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &TokenizerBase::out()
{
    return *d_out;
}

inline void TokenizerBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void TokenizerBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<TokenizerBase::StreamStruct> const &TokenizerBase::streamStack() const
{
...
inline void TokenizerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void TokenizerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &TokenizerBase::matched() const
{
    return d_matched;
}

inline std::string const &TokenizerBase::cwd() const
{
    return d_cwd;
}

inline std::string const &TokenizerBase::filename() const
{
    return d_filename;
}

inline void TokenizerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t TokenizerBase::length() const
{
    return d_matched.size();
}

inline void TokenizerBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t TokenizerBase::lineNr() const
{
    return d_input->lineNr();
}

inline void TokenizerBase::more()
{
    d_more = true;
}

inline size_t TokenizerBase::state_() const
{
    return d_state;
}

inline size_t TokenizerBase::get_()
{
    return (this->*d_get)();
}

inline size_t TokenizerBase::getInput()
{
    return d_input->get();
}

inline bool TokenizerBase::return_()
{
    return d_return;
}

inline void TokenizerBase::noReturn_()
{
    d_return = false;
}


#endif //  TokenizerBASE_H_INCLUDED
